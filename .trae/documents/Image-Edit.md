...
1. 架构总览
   本方案基于 Vue 3 (Composition API) + Fabric.js (WebGL Backend) 构建，专为纯图片裁剪、进阶调色、结构化标注（画框/水印/马赛克）及客户端高性能压缩而设计。
   系统核心原则是数据与视图解耦、非破坏性编辑流程（Non-destructive Pipeline）以及多图层合成机制。
   +-------------------------------------------------------------------------+
   | Vue 3 UI Layer |
...
2. 状态管理与图层隔离
   2.1 响应式隔离方案
   Fabric.js 内部存在复杂的双向指针引用及循环结构。若直接使用 Vue 的 ref() 或 reactive() 深度代理，会造成 Proxy 拦截所有内部属性，引发显著的渲染卡顿与内存泄漏。
...
Layer 4 交互蒙版 (Overlay) 裁剪框、九宫格线、悬浮条 捕获最高优先级 Pointer 事件 不参与最终图像合成导出 3. 进阶色彩调节系统 (WebGL Pipeline)
系统启用 WebGL 滤镜后端，通过单阶段自定义片段着色器（Fragment Shader）与 1D 纹理查找表（LUT）实现多维度调色。
...
export const MasterColorAdjustmentFilter = fabric.util.createClass(fabric.Image.filters.BaseFilter, {
...
为避免普通贝塞尔样条在极值点处产生的插值震荡（Runge's Phenomenon），采用单调三次样条计算 0 \sim 255 查找表：
...
const t3 = t2 * t;
    ...
    const val = h00 * y[pIdx] + h10 * h * tangents[pIdx] + h01 * y[pIdx + 1] + h11 * h * tangents[pIdx + 1];
...
4.4 矢量化可重编辑局部马赛克
将马赛克实现为克隆底图 + 像素化滤镜 + 裁剪路径蒙版的复合体，支持用户随时点击移动、拉伸与删除：
export async function addMosaicRegion(canvas, baseImage, { left, top, width, height, blockSize = 8 }) {
...
5. 高保真合成导出与压缩管线
   5.1 全分辨率还原导出机制
   为保障大图（如 4000 \times 3000）在视口缩放编辑后不失真，导出时依据原始物理像素动态计算放大因子 M：
   $$M = \frac{\text{Image}_{\text{originWidth}}}{\text{Image}_{\text{currentRenderWidth}}}$$
...
5.2 客户端二次压缩策略
采用 Web Worker + 现代图像编码接口进行多线程异步压缩，避免占用主线程渲染帧：
...
function updateColorPipeline() {
  ...
  const defaultCurvePoints = [{ x: 0, y: 0 }, { x: 255, y: 255 }];
...
7. 性能优化与边界防御清单
1. 纹理尺寸限制保护： 在载入超大图片（如超过 GPU MAX_TEXTURE_SIZE，通常为 4096\text{px} 或 8192\text{px}）时，前置做离屏等比降采样，防止 WebGL 纹理绑定失败崩溃。
1. 绘制节流与 RAF 调度： 调色滑块的 @input 事件严禁用全量 renderAll() 阻塞，统一使用 Fabric 内置的 requestRenderAll()，让更新对齐浏览器显示刷新率（60/120Hz）。
1. 内存即时释放： 在组件注销（onBeforeUnmount）或图片切换时，显式调用 fabricCanvas.dispose()，并遍历调用滤镜和 Pattern 的销毁逻辑，释放绑定的 WebGL 纹理单元（gl.deleteTexture）。
