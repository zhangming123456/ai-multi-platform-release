    ...
    <el-tabs :model-value="props.activeTab" @update:model-value="onTabChange">
      ...
      <el-tab-pane name="operator" label="运算符说明">
        ...
        <ul class="doc-list">
          ...
          <li>
            命中「运算符限定」规则时，条件行中只展示允许使用的运算符，已有条件会自动切换为允许项
          </li>
      ...
      <el-tab-pane name="rule" label="互斥与关联规则说明">
        <ul class="doc-list">
          ...
          <li>
            「运算符限定」规则命中后，目标变量的运算符下拉只展示规则内的运算符（每条规则至少选一个），
            条件行已使用其他运算符时会自动改为第一个允许运算符
          </li>
          <li>多条「运算符限定」规则同时命中时取交集，交集为空则该变量暂无可选运算符</li>
          <li>
